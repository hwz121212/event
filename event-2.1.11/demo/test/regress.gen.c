    ...
    const char * value)
{
  ...
  if ((msg->from_name_data = _strdup(value)) == NULL)
    return (-1);
  msg->from_name_set = 1;
  return (0);
}
    ...
    const char * value)
{
  ...
  if ((msg->to_name_data = _strdup(value)) == NULL)
    return (-1);
  msg->to_name_set = 1;
  return (0);
}
    ...
    const char * value)
{
  ...
  if ((msg->weapon_data = _strdup(value)) == NULL)
    return (-1);
  msg->weapon_set = 1;
  return (0);
}
    ...
    const char * value)
{
  ...
  if ((msg->action_data = _strdup(value)) == NULL)
    return (-1);
  msg->action_set = 1;
  return (0);
}
...
run_notes_add(struct run *msg, const char * value)
{
  ...
  if (value != NULL) {
    msg->notes_data[msg->notes_length - 1] = _strdup(value);
    ...
    const char * value)
{
  if (msg->how_data != NULL)
    free(msg->how_data);
  if ((msg->how_data = _strdup(value)) == NULL)
    return (-1);
  msg->how_set = 1;
  return (0);
}
    ...
    const char * value)
{
...

  {
    ...
    msg->notes_data[off] = _strdup(value);
    if (msg->notes_data[off] == NULL) {
